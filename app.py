...
loaded_model = load(open('k_means.pkl', 'rb'))
...
def Cluster_prediction(input_data):
    ...
    scaler = MinMaxScaler()
    input_data_normalized = scaler.fit_transform(input_data_as_numpy_array)

# Reshape the array for prediction
    input_data_reshaped = input_data_normalized.reshape(1, -1)
...
def main():
    ...
    # giving a title
    st.markdown("<h1 style='color:green;'>Predicting the state of Development of a Country</h1>",
                unsafe_allow_html=True)
    st.markdown("<h5 style='color:blue;'>Check the Values</h5>",
                unsafe_allow_html=True)
    st.sidebar.markdown(
        "<h5 style='color: blue ;'>Enter the necessary details to find whether the Country is Developed or Underdeveloped or Developing country</h5>", unsafe_allow_html=True)
    st.sidebar.markdown(
    ...
    Birth_Rate = st.sidebar.number_input(
        'Birth Rate (0.000-1.000)', step=0.001, format="%.3f")
    Business_Tax_Rate = st.sidebar.number_input(
        'Business Tax Rate (Do not include "%")')
    ...
    Health_Exp_GDP = st.sidebar.number_input(
        'Health Exp % GDP (Do not include "%")')/100
    Health_Exp_Capita = st.sidebar.number_input('Health Exp/Capita ($)')
    Hours_to_do_Tax = st.sidebar.number_input('Hours to do Tax')
    Infant_Mortality_Rate = st.sidebar.number_input(
        'Infant Mortality Rate (0.000-1.000)', step=0.001, format="%.3f")
    Internet_Usage = st.sidebar.number_input('Internet Usage')
    Lending_Interest = st.sidebar.number_input('Lending Interest')
    Life_Expectancy_Female = st.sidebar.number_input(
        'Life Expectancy Female (Age)')
    ...
    Population_0_14 = st.sidebar.number_input(
        'Percentage of Population between 0-14 (Do not include "%")')/100
    Population_15_64 = st.sidebar.number_input(
        'Percentage of Population between 15-64 (Do not include "%")')/100
    Population_65_and_above = st.sidebar.number_input(
        'Population percentage of people above the age of 65 (Do not include "%")')/100
    Population_Total = st.sidebar.text_input('Total population')
    Population_Urban = st.sidebar.number_input(
        'Population Urban % (Do not include "%")')/100
    ...
    if st.button('Predict'):
        Predict = Cluster_prediction([[Birth_Rate, Business_Tax_Rate, CO2_Emissions, Days_to_Start_Business, Energy_Usage, GDP, Health_Exp_GDP, Health_Exp_Capita, Hours_to_do_Tax, Infant_Mortality_Rate, Internet_Usage,
        ...
        st.write(
            f'This is a <span style="color:{color}">{Predict}</span>.', unsafe_allow_html=True)
